from bingo_sort import bingo               # Импорт модуля сортировки бинго
from bose_nelson_sort import bose_nelson   # Импорт модуля сортировки Боуза-Нельсона
from ins_sort import insertion             # Импорт модуля соритровки вставками
from pancake_sort import pancake           # Импоррт модуля блинной сортировки
...
class Application(tk.Frame):
    ...
    def create_inmatrx_win(self):
        if self.matrix_N.get().isdigit() and self.matrix_M.get().isdigit() and int(self.matrix_N.get()) >= 1 and int(self.matrix_M.get().isdigit()) >= 1:
            ...
            tk.Button(self.inmatrx_win, text="QUIT", fg="red", command=lambda x=1: self.new_start(x)).grid(row=2, columnspan=2)
    ...
    def choise_sort_win(self):
        ...
        tk.Button(self.choise_win, text='Вставки', command=self.insertion_sort).grid(row=1, column=0, sticky='nswe', padx=4)
        tk.Button(self.choise_win, text='Блинная сортировка', command=self.pancake_sort).grid(row=1, column=1, sticky='nswe', padx=4)
        tk.Button(self.choise_win, text="QUIT", fg="red", command=lambda x=2: self.new_start(x)).grid(row=2, columnspan=2, sticky='nswe', padx=4)
